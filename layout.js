(function () {
  ...
  function styles() {
    ...
    s.textContent = [
      ...
      ".tiny{font-size:11px;letter-spacing:.02em;color:#8d8d8d}",
      ".ex-name{font-size:16px;font-weight:700;letter-spacing:-.02em}",
      ...
      "#view-home .backup-due,.backup-banner{display:flex;align-items:center;justify-content:space-between;background:#141414;border:1px solid #1e1e1e;border-radius:14px;padding:10px 12px;margin:0 0 10px;font-size:13px}",
      ".chip{background:#161616;border:1px solid #222;min-height:36px;padding:7px 12px}",
      ".chips{margin:8px 0 12px;scrollbar-width:none}",
      ".chips::-webkit-scrollbar{display:none}",
      "#view-library .card,#view-history .card{display:block}",
      ...
      "#view-workout .card > .btn.warn,#view-workout .card > [data-act='remove-ex']{position:absolute;top:10px;right:10px;width:34px!important;min-width:34px;max-width:34px;padding:4px;min-height:34px;font-size:14px}",
      ...
      "#view-workout .step-wrap{gap:3px}",
      ...
      "#view-workout.empty-train .sess-strip{opacity:.7}",
  ...
  function hideJunk(view) {
    ...
    Array.prototype.slice.call(view.querySelectorAll("div,p,section")).forEach(function (el) {
      if (el.querySelector(".card,.stats,[data-tpl-id]")) return;
      var t = (el.textContent || "").replace(/\s+/g, " ").trim();
      if (/^backup due/i.test(t) && !el.classList.contains("backup-banner")) {
        el.className = (el.className + " backup-banner").trim();
      }
    });
  }
  function tidyLog() {
  ...
  function tidyTrain() {
    ...
    Array.prototype.slice.call(view.querySelectorAll(".card")).forEach(function (card) {
      if (card.querySelector(".set-grid")) return;
